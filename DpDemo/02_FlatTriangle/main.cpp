...
class FlatTriangle : public DemoApp
{
public:
...
void FlatTriangle::OnUpdate(const Timestep& timestep)
{
	if (++angleY_ >= 360.0f)
		angleY_ = 0.0f;
...
void FlatTriangle::OnRender(const Timestep& timestep)
{
...
int main()
{
	Ref<FlatTriangle> app(new FlatTriangle);
	app->Create(400, 400, "Flat Triangle");
	app->Loop();
